...
export async function createBarrels(level: Level, pos: THREE.Vector3) {
  console.log("create Barrels");
  // Create a group to hold the barrels
  const barrelsGroup = new THREE.Group();
  const loader = new GLTFLoader();
  const barrelsModel = (
    await loader.loadAsync("/models/gltf/barrels.gltf")
  ).scene.clone();
  barrelsModel.scale.set(4, 4, 4);
  barrelsGroup.add(barrelsModel);
  // Enable shadows for all meshes in the model
  barrelsGroup.traverse((child: THREE.Object3D) => {
    if (child instanceof THREE.Mesh) {
      child.castShadow = true;
      child.receiveShadow = true;
    }
  });
  barrelsGroup.position.set(pos.x, pos.y, pos.z);
  // Add to scene
  level.scene.add(barrelsGroup);
